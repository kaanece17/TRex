...
INPUT_CSV = ROOT / "outputs" / "formula_research_reference" / "bist_earnings_quality_weight_scale_monthly.csv"
OUTPUT_DIR = ROOT / "outputs" / "formula_research_reference"
PROFILES = ["baseline", "and_scale_050", "or_scale_075", "and_scale_075"]
...
def _rolling_rows(profile_id: str, monthly: pd.DataFrame, window: int) -> list[dict[str, object]]:
    ...
    months = monthly["month"].astype(str).reset_index(drop=True)
...
def main() -> None:
    ...
    monthly_all = pd.read_csv(INPUT_CSV)
    monthly_all["month"] = monthly_all["month"].astype(str)
    monthly_all["net_return"] = pd.to_numeric(monthly_all["net_return"], errors="coerce")
    monthly_all = monthly_all[monthly_all["variant"].isin(PROFILES)].copy()

    monthly_map = {
        profile: monthly_all[monthly_all["variant"] == profile][["month", "net_return"]].reset_index(drop=True)
        for profile in PROFILES
    }
    ...
    aligned_returns = {profile: monthly_map[profile]["net_return"] for profile in PROFILES}
    ...
    rolling.to_csv(OUTPUT_DIR / "bist_earnings_quality_weight_scale_robustness_rolling.csv", index=False)
    rolling_summary.to_csv(OUTPUT_DIR / "bist_earnings_quality_weight_scale_robustness_rolling_summary.csv", index=False)
    bootstrap.to_csv(OUTPUT_DIR / "bist_earnings_quality_weight_scale_robustness_bootstrap.csv", index=False)
    bootstrap_summary.to_csv(OUTPUT_DIR / "bist_earnings_quality_weight_scale_robustness_bootstrap_summary.csv", index=False)

    lines = [
        "# BIST Earnings Quality Weight Scale Robustness",
        "",
        "Method:",
    ...
    (OUTPUT_DIR / "bist_earnings_quality_weight_scale_robustness_readout.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
