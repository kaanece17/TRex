...
PROFILES = [
    "us_industrials_momentum",
    "us_industrials_quality_scorecap",
    "us_industrials_quality_earnings",
]
...
def main() -> None:
    ...
    rolling.to_csv(OUTPUT_DIR / "us_live_candidate_robustness_rolling.csv", index=False)
    rolling_summary.to_csv(OUTPUT_DIR / "us_live_candidate_robustness_rolling_summary.csv", index=False)
    bootstrap.to_csv(OUTPUT_DIR / "us_live_candidate_robustness_bootstrap.csv", index=False)
    bootstrap_summary.to_csv(OUTPUT_DIR / "us_live_candidate_robustness_bootstrap_summary.csv", index=False)

    lines = [
        "# US Live Candidate Robustness",
        "",
        "Method:",
        "- Rolling windows: 12m, 24m, 36m",
        "- Block bootstrap: 6-month blocks, 2000 simulations",
        "",
    ...
    (OUTPUT_DIR / "us_live_candidate_robustness_readout.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
