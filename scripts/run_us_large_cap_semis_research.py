...
CONFIG_PATH = Path("/Users/kaanece/projects/TRex/config.us_large_cap_semis_quality_earnings.yaml")
...
SUMMARY_CSV = OUTPUT_DIR / "us_large_cap_semis_research_summary.csv"
MONTHLY_CSV = OUTPUT_DIR / "us_large_cap_semis_research_monthly.csv"
README_MD = OUTPUT_DIR / "us_large_cap_semis_research_readout.md"
...
CANDIDATES = [
    CandidateSpec(
        name="semis_quality_earnings",
        notes="Large-cap semis with full quality plus earnings sleeve.",
        formula="quality_plus_earnings",
    ),
    CandidateSpec(
        name="semis_quality_op_growth",
        notes="Large-cap semis with quality plus operating-profit growth sleeve.",
        formula="quality_plus_op_growth",
    ),
    CandidateSpec(
        name="semis_quality_op_growth_w15",
        notes="Large-cap semis with heavier operating-profit growth sleeve.",
        formula="quality_plus_op_growth",
        earnings_weight=1.5,
    ),
]
...
def main() -> None:
    ...
    baseline = summary.loc[summary["candidate"] == "semis_quality_earnings"].iloc[0]
    summary["strict_pass"] = (
    ...
    top = summary.iloc[0]
    lines = [
        "# US Large-Cap Semis PIT Research",
        "",
        "Universe choice:",
        "- Current static S&P 500 Semiconductors + Semiconductor Materials & Equipment.",
        "",
        f"- Baseline candidate: `semis_quality_earnings`",
