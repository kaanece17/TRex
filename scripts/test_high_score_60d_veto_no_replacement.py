...
def _simulate_no_replacement(selected: pd.DataFrame, baseline_monthly: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    base = selected.copy()
    adjusted_rows = []
    portfolio_value = INITIAL_CAPITAL
    for month, month_df in base.groupby("month", sort=True):
        surviving = month_df[~month_df["vetoed"]].copy()
        adjusted_return = float((surviving["weight"] * surviving["net_return"]).sum()) if not surviving.empty else 0.0
        start_value = portfolio_value
        portfolio_value = portfolio_value * (1 + adjusted_return)
        adjusted_rows.append(
            {
                "month": month,
                "baseline_month_return": float(
                    baseline_monthly.loc[baseline_monthly["month"] == month, "net_return"].iloc[0]
                ),
                "adjusted_month_return": adjusted_return,
                "return_delta": adjusted_return
                - float(baseline_monthly.loc[baseline_monthly["month"] == month, "net_return"].iloc[0]),
    ...
    adjusted_monthly = pd.DataFrame(adjusted_rows)
    return base, adjusted_monthly
...
def _write_readout(run_id: str, summary: pd.DataFrame, monthly: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "yerine alt listeden aday koyma; bos kalan agirlik nakitte kalsin.",
        "",
        "Donem ozeti:",
    ]
    ...
    for row in best.to_dict(orient="records"):
        lines.append(
            f"- {row['month']}: delta={row['return_delta']:.4f}, vetoed={row['vetoed_symbols']}"
        )
    ...
    (OUTPUT_DIR / "accepted_top6_high_score_60d_veto_no_replacement_readout.md").write_text(
        "\n".join(lines) + "\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    selected, adjusted_monthly = _simulate_no_replacement(selected, baseline_monthly)
    summary = _period_summary(adjusted_monthly)

    selected.to_csv(OUTPUT_DIR / "accepted_top6_high_score_60d_veto_no_replacement_selected.csv", index=False)
    adjusted_monthly.to_csv(
        OUTPUT_DIR / "accepted_top6_high_score_60d_veto_no_replacement_monthly.csv",
        index=False,
    )
    summary.to_csv(
        OUTPUT_DIR / "accepted_top6_high_score_60d_veto_no_replacement_summary.csv",
        index=False,
    )
