...
SUMMARY_CSV = OUTPUT_DIR / "bist_formula_combination_summary.csv"
MONTHLY_CSV = OUTPUT_DIR / "bist_formula_combination_monthly.csv"
README_MD = OUTPUT_DIR / "bist_formula_combination_readout.md"
...
CANDIDATES = [
    CandidateSpec("sum_baseline", "Current accepted combination: X1 + X2.", "note_best_fit"),
    CandidateSpec("product_combo", "More sensitive combination: X1 * X2.", "note_best_fit_product"),
    CandidateSpec("signed_sqrt_product_combo", "Softer product: sign(X1*X2) * sqrt(|X1*X2|).", "note_best_fit_signed_sqrt_product"),
    CandidateSpec("harmonic_combo", "Harmonic-style blend: 2*X1*X2/(X1+X2).", "note_best_fit_harmonic"),
]
...
def main() -> None:
    ...
    top = summary.iloc[0]
    lines = [
        "# BIST Formula Combination Research",
        "",
