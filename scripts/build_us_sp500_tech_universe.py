...
UNIVERSE_NAME = "US_LARGE_CAP_TECH"
USER_AGENT = "Mozilla/5.0 (compatible; TRex US tech universe builder)"
OUTPUT_SYMBOLS = Path("/Users/kaanece/projects/TRex/data/universe/us_large_cap_tech_symbols.csv")
OUTPUT_MEMBERSHIP = Path("/Users/kaanece/projects/TRex/data/universe/us_large_cap_tech_membership.csv")


def _load_sector_from_page(label: str, url: str, sector_name: str) -> pd.DataFrame:
    ...
    for row in rows[1:]:
        ...
        if not symbol:
            continue
        records.append({"symbol": symbol, "source_label": label, "source_url": url, "sector": sector})
    return pd.DataFrame(records)


def main() -> None:
    label, url = SOURCE_PAGE
    combined = _load_sector_from_page(label, url, TARGET_SECTOR)
    ...
    print(f"universe={UNIVERSE_NAME} sector={TARGET_SECTOR} symbols={len(symbols)} membership={len(membership)}")
