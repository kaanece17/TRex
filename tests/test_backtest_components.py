...
class TestBuildEqualWeightPositions:
    def test_buildEqualWeightPositions_lessThanTopN_returnsEqualWeights(self):
        selected = pd.DataFrame([{"symbol": "AAA"}, {"symbol": "BBB"}])
        expected = [0.5, 0.5]

        result = build_equal_weight_positions(selected)

        assert result["weight"].tolist() == expected

    def test_buildEqualWeightPositions_emptySelection_returnsEmptyPositions(self):
...
class TestCalculatePositionReturnOpenToOpen:
    def test_calculatePositionReturnOpenToOpen_validPrices_returnsNetReturnAfterCommission(self):
        ...
        result = calculate_position_return_open_to_open(prices, "AAA", date(2024, 5, 2), date(2024, 5, 31), 0.001)
