...
class TestRunMonthlyRotationBacktest:
    ...
    def test_runMonthlyRotationBacktest_rebalanceOpenToOpen_usesNextMonthOpenAsExit(self):
        ...
        assert may_row["net_return"] == pytest.approx(0.199)
    ...
    def test_runMonthlyRotationBacktest_retainedPosition_waivesRolloverCommission(self):
        ...
        assert jun_position["sell_commission_rate"] == pytest.approx(0.0)

    def test_runMonthlyRotationBacktest_rebalanceOpenToOpen_keepsTailMonthAsPlannedOnly(self):
        config = BacktestConfig.model_validate(
            {
                "project": {"name": "test", "timezone": "Europe/Istanbul"},
                "data": {"storage": "duckdb", "duckdb_path": ":memory:"},
        ...
        assert result["monthly_results"]["month"].tolist() == ["2024-05", "2024-06"]
        assert result["selected_positions"]["month"].tolist() == ["2024-05", "2024-06"]
        assert result["planned_positions"]["month"].tolist() == ["2024-05", "2024-06", "2024-07"]
